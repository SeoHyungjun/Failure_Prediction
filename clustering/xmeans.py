...
if __name__ == "__main__":
	...
	# データの準備
	x = np.array([np.random.normal(loc, 0.4, 100) for loc in np.repeat([1,2,2], 2)]).flatten()
	y = np.array([np.random.normal(loc, 0.3, 100) for loc in np.tile([4,2,3], 2)]).flatten()
