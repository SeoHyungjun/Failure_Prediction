...
def sel_rows_by_index(data, start_index, end_index):
...
def sel_cols_by_index(data, cols_index):
    """
...
def drop_cols_by_index(data, cols_index):
    """
    input : cols_index is integer or list of integer.
    output : dataframe
    """
...
def sel_cols_by_name(data, cols_name):
    """
...
def drop_cols_by_name(data, cols_name):
    """
    input : cols_name is string or list of string.
    output : dataframe
    """
...
def split_xy_by_yindex(data, y_cols=-1):
    """
    if y_cols is -1, y is last column
    output : dataframe
    """
    x = drop_cols_by_index(data, y_cols)
    y = sel_cols_by_index(data, y_cols)
    return x,y

def split_xy_by_yname(data, y_cols):
    """
    output : dataframe
    """
    x = drop_cols_by_name(data, y_cols)
    y = sel_cols_by_name(data, y_cols)
    return x,y

def create_window_data(data, y_cols, window_size, lead_time, strides=1):
    """
    ...
    if isinstance(y_cols, list):
        if isinstance(y_cols[0], int):
            orig_x, orig_y = split_xy_by_yindex(data, y_cols)
        elif isinstance(y_cols[0], str):
            orig_x, orig_y = split_xy_by_yname(data, y_cols)
